...
export default class AtendimentosController {
        ...
        try {
            ...
            if (atendimentoBuscado === null) {
                return response.status(404).json({
                    message: `Atendimento com ID - ${id} não encontrado.`,
                });
            } else {
                return response.status(200).json({
                    message: 'Operação bem sucedida!',
    ...
    static async addAtendimento (request, response) {
        ...
        try {
            const pacienteBuscado = await PacienteRepository.findByPk(paciente_id);
            if (pacienteBuscado === null) {
                ...
                const exibirAtendimentoCriado = await AtendimentoRepository.findByPk( 
                    id, 
                    {
                        include: [
                            {
